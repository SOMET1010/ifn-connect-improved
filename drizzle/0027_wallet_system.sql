/*
  ...
  - Users can ONLY update their own wallet balance (through controlled procedures)
  - System can create wallets for new users

  ### Wallet Transactions Security
  - Users can view transactions where they are sender OR receiver
  - Users can create transactions from their own wallet
  - Users can update their own pending transactions (cancel, complete)

  ### Beneficiaries Security
  - Users can ONLY view, create, update, delete their own beneficiaries
  - No access to other users' beneficiary lists
...
CREATE POLICY "Users can view own transactions"
  ...
  TO authenticated
  USING (
    from_user_id = auth.uid()::integer
    OR to_user_id = auth.uid()::integer
  );

CREATE POLICY "Users can create transactions from own wallet"
